...
function makeDistroChart(settings) {
    ...
    function tooltipHover(groupName, metrics) {
        ...
        return function () {
            chart.objs.tooltip.transition().duration(200).attr("opacity", 0.9);
    ...
    !function prepareSettings() {
        ...
        chart.xScale = d3.scaleOrdinal().domain(Object.keys(chart.groupObjs)).range([0, chart.width]);
    ...
    chart.update = function () {
        ...
        chart.width = parseInt(chart.objs.chartDiv.attr("width"), 10) - (chart.margin.left + chart.margin.right);
        chart.height = parseInt(chart.objs.chartDiv.attr("height"), 10) - (chart.margin.top + chart.margin.bottom);
    ...
    !function prepareChart() {
        ...
        chart.objs.mainDiv = d3.select(chart.settings.selector)
            .attr("max-width", chart.divWidth + "px");
        // Add all the divs to make it centered and responsive
        chart.objs.mainDiv.append("div")
            .attr("class", "inner-wrapper")
            .attr("padding-bottom", (chart.divHeight / chart.divWidth) * 100 + "%")
            ...
            for (cName in chart.groupObjs) {
                ...
                if (vOpts.showViolinPlot !== false) {
                    ...
                    cViolinPlot.objs.left.area = cViolinPlot.objs.left.g.append("path")
                        .attr("class", "area")
                        .attr("fill", chart.violinPlots.color(cName));
                    ...
                    cViolinPlot.objs.left.line = cViolinPlot.objs.left.g.append("path")
                        .attr("class", "line")
                        .attr("fill", 'none')
                        .attr("stroke", chart.violinPlots.color(cName));
            ...
            for (cName in chart.groupObjs) {
                ...
                if (bOpts.showBox) {
                    cBoxPlot.objs.box = cBoxPlot.objs.g.append("rect")
                        .attr("class", "box")
                        .attr("fill", chart.boxPlots.colorFunct(cName))
                        .attr("stroke", chart.boxPlots.colorFunct(cName));
            ...
            for (cName in chart.groupObjs) {
                ...
                if (nOpts.showNotchBox) {
                    cNotch.objs.notch = cNotch.objs.g.append("polygon")
                        .attr("class", "notch")
                        .attr("fill", chart.notchBoxes.colorFunct(cName))
                        .attr("stroke", chart.notchBoxes.colorFunct(cName));
                ...
                if (cPlot.objs.points) {
                    if (dOpts.plotType == 'beeswarm') {
                        ...
                        var yPtScale = chart.yScale.copy()
                            .range([Math.floor(chart.yScale.bandwidth()[0] / dOpts.pointSize), 0])
            ...
            if (dOpts.showLines && dOpts.showLines.length > 0) {
                ...
                for (var line in dOpts.showLines) {
                    ...
                    chart.dataPlots.objs.lines[cMetric].g = chart.dataPlots.objs.g.append("path")
                        ...
                        .attr("fill", 'none')
                        .attr("stroke", chart.colorFunct(cMetric));
                }
                ...
                if (dOpts.showPlot) {
                    ...
                    for (var pt = 0; pt < chart.groupObjs[cName].values.length; pt++) {
                        cPlot.objs.points.pts.push(cPlot.objs.points.g.append("circle")
                            ...
                            .attr("fill", chart.dataPlots.colorFunct(cName))
                ...
                if (dOpts.showBeanLines) {
                    ...
                    for (var pt = 0; pt < chart.groupObjs[cName].values.length; pt++) {
                        cPlot.objs.bean.lines.push(cPlot.objs.bean.g.append("line")
                            ...
                            .attr("stroke-width", '1')
                            .attr("stroke", chart.dataPlots.colorFunct(cName)));
